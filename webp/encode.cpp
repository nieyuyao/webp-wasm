...
thread_local val Uint8ClampedArray = val::global("Uint8ClampedArray");
...
val encodeRGB(std::string rgb, int width, int height, int quality_factor) {
	...
	return Uint8ClampedArray.new_(typed_memory_view(size, output));
}
...
val encode(std::string data, int width, int height, bool has_alpha, SimpleWebPConfig config)
{
	...
	val encoded_data = Uint8ClampedArray.new_(typed_memory_view(wrt.size, wrt.mem));
	WebPPictureFree(&pic);
...
val encodeAnimation(int width, int height, bool has_alpha, val durations, std::string data)
{
	...
	auto timestamps = vecFromJSArray<int>(durations);
	...
	for (int i = 0; i < frames; i++)
	{
		...
		int success = WebPAnimEncoderAdd(enc, &pic, timestamps[i], &config);
		if (!success) {
			return val::null();
		}
	}
	...
	val encoded_data = Uint8ClampedArray.new_(typed_memory_view(webp_data.size, webp_data.bytes));
